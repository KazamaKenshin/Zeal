class_name PoliceVehicleTest
extends VehicleBody3D

# follows roadlanes normally but towards the players moving direction: ie if fwd then fwd
# if close enough starts ramming the player
# if ahead starts braking to stop the player
...
func get_closest_lane(max_distance: float):
	...
	for lane in all_lanes:
		...
		if this_lane_dist > max_distance:
			continue
		elif closest_lane == null or this_lane_dist < closest_dist:
			closest_lane = lane
...
func _set_interest_and_next_lane():
	...
	var player_direction = player_pos - car.global_transform.origin

	for i in range(num_rays):
		var d = ray_directions[i].rotated(Vector3(0, 1, 0), car.rotation.y).dot(path_direction)
		var player_d = ray_directions[i].rotated(Vector3(0, 1, 0), car.rotation.y).dot(player_direction)
		interest[i] = max(0, d) + max(0, player_d)
...
func _physics_process(delta):
	...
	if brake_lights:
		brake_lights.mesh.surface_get_material(0).emission_energy_multiplier = 5 if brake_val != 0.0 else 0.5

...
func _physics_process(_delta):
	...
	#rb26dett
	pitch = get_parent().rpm / 7700.0

	volume = 1
	fade = (get_node("100500").pitch_scale  -0.22222)*(crossfade_influence + 1)
	...
	for i in get_children():
		var maxvol = float(str(i.get_child(0).get_name())) / 100.0
		var maxpitch = float(str(i.get_name())) / 100000.0
